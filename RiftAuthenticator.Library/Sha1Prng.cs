...
namespace Org.Apache.Harmony.Security.Provider.Crypto
{
    class Sha1Prng
    {
        ...
        private static readonly uint[] END_FLAGS = { 0x80000000, 0x800000, 0x8000, 0x80 };

        private static readonly int[] RIGHT1 = { 0, 40, 48, 56 };

        private static readonly int[] RIGHT2 = { 0, 8, 16, 24 };

        private static readonly int[] LEFT = { 0, 24, 16, 8 };

        private static readonly uint[] MASK = { 0xFFFFFFFF, 0x00FFFFFF, 0x0000FFFF, 0x000000FF };
        ...
        private const int HASHBYTES_TO_USE = 20;
        ...
        private const int COUNTER_BASE = 0;

        private const int HASHCOPY_OFFSET = 0;

        private const int EXTRAFRAME_OFFSET = 5;

        private const int FRAME_OFFSET = 21;

        private const int MAX_BYTES = 48;
         ...
         */
        private const int HASH_OFFSET = 82;
        ...
        private System.Security.Cryptography.SHA1Managed _Digest = new System.Security.Cryptography.SHA1Managed();
        ...
        static void computeHash(uint[] arrW)
        {
            ...
            uint a = arrW[HASH_OFFSET];
            uint b = arrW[HASH_OFFSET + 1];
            uint c = arrW[HASH_OFFSET + 2];
            uint d = arrW[HASH_OFFSET + 3];
            uint e = arrW[HASH_OFFSET + 4];
         ...
         */
        static void updateHash(uint[] intArray, byte[] byteInput, int fromByte, int toByte)
        {

            // As intArray contains a packed bytes
            // the buffer's index is in the intArray[BYTES_OFFSET] element

            int index = (int)intArray[BYTES_OFFSET];
            int i = fromByte;
            int maxWord;
            int nBytes;
            ...
            {       // appending bytes in non-full word (as if)
                ...
                {
                    intArray[wordIndex] |= (uint)((byteInput[i] & 0xFF) << ((3 - byteIndex) << 3));
                    byteIndex++;
                }
                if (byteIndex == 4)
                {
            ...
            for (int k = 0; k < maxWord; k++)
            {
                ...
                intArray[wordIndex] = (((uint)byteInput[i] & 0xFF) << 24) |
                                      (((uint)byteInput[i + 1] & 0xFF) << 16) |
                                      (((uint)byteInput[i + 2] & 0xFF) << 8) |
                                      (((uint)byteInput[i + 3] & 0xFF));
                i += 4;
                wordIndex++;
            ...
            if (nBytes != 0)
            {
                ...
                uint w = ((uint)byteInput[i] & 0xFF) << 24;

                if (nBytes != 1)
                {
                    w |= ((uint)byteInput[i + 1] & 0xFF) << 16;
                    if (nBytes != 2)
                    {
                        w |= ((uint)byteInput[i + 2] & 0xFF) << 8;
                    }
                }
         ...
         */
        private void engineSetSeed(byte[] seed)
        {
            if (seed == null)
                throw new ArgumentNullException("seed");
            if (state == State.NextBytes)
                Array.Copy(copies, HASHCOPY_OFFSET, this.seed, HASH_OFFSET, EXTRAFRAME_OFFSET);
            state = State.SetSeed;
            if (seed.Length != 0)
                UpdateSeed(seed);
        }
         ...
         */
        private void engineNextBytes(byte[] bytes, int bytesOffset, int bytesLength)
        {

            int n;

            ulong bits; // number of bits required by Secure Hash Standard
            int nextByteToReturn; // index of ready bytes in "bytes" array
            uint lastWord; // index of last word in frame containing bytes
            const int extrabytes = 7;// # of bytes to add in order to computer # of 8 byte words
            ...
            else if (state == State.SetSeed)
            {

                Array.Copy(seed, HASH_OFFSET, copies, HASHCOPY_OFFSET, EXTRAFRAME_OFFSET);
                ...
                for (uint i = lastWord + 3; i < FRAME_LENGTH + 2; i++)
                {
                    seed[i] = 0;
                }

                bits = ((ulong)seedLength << 3) + 64; // transforming # of bytes into # of bits
                ...
                {
                    seed[14] = (uint)(bits >> 32);
                    seed[15] = (uint)(bits & 0xFFFFFFFF);
                }
                else
                {
                    copies[EXTRAFRAME_OFFSET + 14] = (uint)(bits >> 32);
                    copies[EXTRAFRAME_OFFSET + 15] = (uint)(bits & 0xFFFFFFFF);
                }

                nextBIndex = HASHBYTES_TO_USE; // skipping remaining random bits
            }
            state = State.NextBytes;
            ...
            n = (HASHBYTES_TO_USE - nextBIndex) < (bytesLength - nextByteToReturn) ? HASHBYTES_TO_USE
                    - nextBIndex
                    : bytesLength - nextByteToReturn;
            if (n > 0)
            {
                Array.Copy(nextBytes, nextBIndex, bytes, nextByteToReturn + bytesOffset, n);
                nextBIndex += n;
                nextByteToReturn += n;
            }

            if (nextByteToReturn >= bytesLength)
            {
            ...
            for (; ; )
            {
                if (n == 0)
                {
                    ...
                    seed[lastWord] = (uint)(counter >> 32);
                    seed[lastWord + 1] = (uint)(counter & 0xFFFFFFFF);
                ...
                else
                {
                    ...
                    seed[lastWord] |= (uint)((counter >> RIGHT1[n]) & MASK[n]);
                    seed[lastWord + 1] = (uint)((counter >> RIGHT2[n]) & 0xFFFFFFFF);
                    seed[lastWord + 2] = (uint)((counter << LEFT[n]) | END_FLAGS[n]);
                }
                if (seed[BYTES_OFFSET] > MAX_BYTES)
                {
                ...
                if (seed[BYTES_OFFSET] > MAX_BYTES)
                {
                    ...
                    Array.Copy(seed, 0, copies, FRAME_OFFSET, FRAME_LENGTH);
                    Array.Copy(copies, EXTRAFRAME_OFFSET, seed, 0, FRAME_LENGTH);

                    computeHash(seed);
                    Array.Copy(copies, FRAME_OFFSET, seed, 0, FRAME_LENGTH);
                }
                counter++;
                ...
                nextBIndex = 0;
                j = HASHBYTES_TO_USE < (bytesLength - nextByteToReturn) ? HASHBYTES_TO_USE
                        : bytesLength - nextByteToReturn;

                if (j > 0)
                {
                    Array.Copy(nextBytes, 0, bytes, nextByteToReturn + bytesOffset, j);
                    nextByteToReturn += j;
                    nextBIndex += j;
                }
        ...
        public Sha1Prng()
        {
            seed = new uint[HASH_OFFSET + EXTRAFRAME_OFFSET];
            seed[HASH_OFFSET + 0] = H0;
            ...
            seedLength = 0;
            copies = new uint[2 * FRAME_LENGTH + EXTRAFRAME_OFFSET];
        ...
        private void DumpSeed(uint[] values)
        {
            ...
            for (int i = 0; i != values.Length; ++i)
            {
                if (i != 0)
                    result.Append("-");
                result.AppendFormat("{0:X08}", values[i]);
            }
        ...
        {
            AddSeedMaterial(BitConverter.GetBytes(seed));
        }
        ...
        {
            engineNextBytes(bytes, start, len);
        }

        public void NextBytes(byte[] bytes)
        {
            engineNextBytes(bytes, 0, bytes.Length);
        }
    }
}
